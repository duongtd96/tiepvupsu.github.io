...
Nhận thấy rằng ở đây có 2 class \\(\text{B}\\) và \\(\text{N}\\), ta cần đi tìm \\(p(\text{B})\\) và \\(p(\text{N})\\). Và dựa trên tần số xuất hiện của mỗi class trong tập training. Ta sẽ có: 
